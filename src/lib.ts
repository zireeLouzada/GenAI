...
export const compactBRL=(value:number)=>{const absolute=Math.abs(value);if(absolute<1000)return `R$ ${new Intl.NumberFormat('pt-BR',{maximumFractionDigits:0}).format(value)}`;return `R$ ${new Intl.NumberFormat('pt-BR',{maximumFractionDigits:1}).format(value/1000)} mil`};
